    ...
    <Row class="operation" v-if="!showOnly">
      ...
      <Dropdown @on-click="handleTreeDropdown">
        ...
        <DropdownMenu slot="list">
          <DropdownItem name="edit">编辑文件夹</DropdownItem>
          <DropdownItem name="del">删除文件夹</DropdownItem>
  ...
  methods: {
    ...
    del() {
      ...
      this.$Modal.confirm({
        ...
        loading: true,
        content:
          "您确认要删除文件夹 " + this.selectNode.title + " 及其所有文件吗?",
        onOk: () => {
          // 删除
          deleteFileCate({ ids: this.selectNode.id }).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.init();
